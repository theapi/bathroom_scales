...
TXState_TypeDef TX_StateMachineRun(TXState_TypeDef state, UART_HandleTypeDef *huart2, LCD_TypeDef *lcd) {
    switch (state) {
        ...
        case TXSTATE_STARTING: return TX_StateStarting(state);
        case TXSTATE_ON: return TX_StateOn(state, huart2, lcd->weight);
        case TXSTATE_TRANSMITTING: return TX_StateTranmitting(state);
        case TXSTATE_SENT: return TX_StateSent(state, huart2);
        case TXSTATE_COMPLETE: return TX_StateComplete(state);
        default: return TX_StateOff(state);
    }
    return state;
}
